...
% Last Modified by GUIDE v2.5 10-Jun-2020 13:40:17
...
%at the beginig we intialiazt the model with the lecture example as following
%last edit 10/6/2020 by girgis micheal
...
set(handles.edit1, 'string','1');%Ts
        ...
        %continuous plant
set(handles.edit2, 'string','0');%delay   %3 0.6
...
set(handles.checkbox2, 'value',0);%continuous design criteria
...
function pushbutton4_Callback(hObject, eventdata, handles)%Generate Automatic Code Files
...
c=clock;
name=sprintf('System_%d%02d%02d_%02d%02d_%02d', c(1), c(2), c(3), c(4), c(5), round(c(6)));
nameofcont=sprintf('System_%d%02d%02d_%02d%02d_%02d_controller', c(1), c(2), c(3), c(4), c(5), round(c(6)));%controller
sim_time=100; t_ref=5; t_dist=50;
str_Ts=num2str(Ts);
x=0; y=0;
sys=new_system(name);
sys2=new_system(nameofcont);%controller
set_param(name, 'stoptime',num2str(sim_time));
set_param(nameofcont, 'stoptime',num2str(sim_time));%controller
x=x+0; y=y+100;%step position
add_block('simulink/Sources/Step', [name '/Reference'], 'position',[x, y, x+30, y+30],...
    'time',num2str(t_ref*Ts), 'sampletime',str_Ts);
x=x+30;%step width
...
x=x+35;%spacing
add_block('simulink/Discrete/Discrete Filter', [nameofcont '/Bm(z)//Am(z)'], 'position',[x, y, x+195, y+30],...
    'numerator',['[' num2str(Bm) ']'], 'denominator',['[' num2str(Am) ']'], 'sampletime',str_Ts);
x=x+195;%tracking width


x=x+35;%spacing
add_block('simulink/Discrete/Discrete Filter', [nameofcont '/T(z)'], 'position',[x, y, x+195, y+30],...
    'numerator',['[' num2str(T) ']'], 'denominator','[1]', 'sampletime',str_Ts);
x=x+195;%T width
add_line(nameofcont, 'Bm(z)//Am(z)/1', 'T(z)/1');

x=x+25;%spacing
add_block('simulink/Math Operations/Sum', [nameofcont '/Sum1'], 'position',[x, y+5, x+20, y+5+20],...
    'inputs','|+-');
x=x+20;%sum width
add_line(nameofcont, 'T(z)/1', 'Sum1/1');

x=x+20;%spacing
add_block('simulink/Discrete/Discrete Filter', [nameofcont '/1//Hs(z)'], 'position',[x, y, x+195, y+30],...
    'numerator','[1]', 'denominator',['[' num2str(Hs) ']'], 'sampletime',str_Ts);
x=x+195;%Hs width
add_line(nameofcont, 'Sum1/1', '1//Hs(z)/1');

x=x+35;%spacing
add_block('simulink/Discrete/Discrete Filter', [nameofcont '/1//S(z)'], 'position',[x, y, x+195, y+30],...
    'numerator','[1]', 'denominator',['[' num2str(S) ']'], 'sampletime',str_Ts);
x=x+195;%S width
add_line(nameofcont, '1//Hs(z)/1', '1//S(z)/1');
%add_line(name, 'Sum1/1', '1//S(z)/1');

x=x+330;%make space for possible continuous plant
add_block('simulink/Math Operations/Sum', [nameofcont '/Sum2'], 'position',[x+100+30, y+5, x+100+50, y+5+20],...
    'inputs','++|');
x=x+20;%sum width

x=x-285;%return till after S
if get(handles.checkbox1, 'value')%continuous plant
    add_block('simulink/Discrete/Zero-Order Hold', [name '/Zero-Order Hold'], 'position',[x-800, y+30, x+35-800, y+60]);
    x=x+35;%ZOH width
    %add_line(name, '1//S(z)/1', 'Zero-Order Hold/1');
    
    x=x+35;%spacing
    add_block('simulink/Continuous/Transport Delay', [name '/Plant Delay'], 'position',[x-800, y+30, x+30-800, y+60],...
        'delaytime',get(handles.edit2, 'string'));
    x=x+30;%delay width
    add_line(name, 'Zero-Order Hold/1', 'Plant Delay/1');
    
    x=x+35;%spacing
    add_block('simulink/Continuous/Transfer Fcn', [name '/Gp(s)'], 'position',[x-800, y+30, x+130-800, y+80],...
        'numerator',get(handles.edit3,'string'), 'denominator',get(handles.edit4,'string'));
    x=x+120;%Gp width
    add_line(name, 'Plant Delay/1', 'Gp(s)/1');
    add_line(name, 'Gp(s)/1', 'Sum2/2');
else%discrete plant
    add_block('simulink/Discrete/Discrete Filter', [nameofcont '/Bp(z)//Ap(z)'], 'position',[x+100, y+5, x+195+100, y+35],...
        'numerator',['[' num2str(Bp) ']'], 'denominator',['[' num2str(Ap) ']'], 'sampletime',str_Ts);
    x=x+195;%plant width
    %add_line(name, '1//S(z)/1', 'Bp(z)//Ap(z)/1');
    add_line(nameofcont, 'Bp(z)//Ap(z)/1', 'Sum2/2');
end

x=x+35;%spacing till sum2
x=x-135;%new branch
add_block('simulink/Sources/Sine Wave', [name '/Disturbance'], 'position',[x-1200, 170, x-1200+30, 200],...
    'amplitude','-0.25', 'frequency',get(handles.edit16,'string'), 'phase','pi/2', 'sampletime',str_Ts);
x=x+30;%disturbance width

x=x+40;%spacing
add_block('simulink/Continuous/Transport Delay', [name '/Disturbance Delay'], 'position',[x-1200, 170, x-1200+30, 200],...
    'delaytime',num2str(t_dist*Ts));
x=x+30;%delay width
add_line(name, 'Disturbance/1', 'Disturbance Delay/1');
...
x=x+125;%spacing from dist. delay till mux
add_block('simulink/Signal Routing/Mux', [name '/Mux'], 'position',[x-800, y, x-800+5, y+38]);
x=x+5;%mux width

x=x+35;%spacing
add_block('simulink/Sinks/Scope', [name '/Scope'], 'position',[x-800, y-5, x-800+30, y+42]);
%add_line(name, '1//S(z)/1', 'Mux/1');
add_line(name, 'Mux/1', 'Scope/1');
%add_line(name, 'Sum2/1', 'Scope/1');
...
%feedback blocks
x=x-465; y=y+110;%from scope left edge to R
block_R=add_block('simulink/Discrete/Discrete Filter', [nameofcont '/R(z)'], 'position',[x, y, x+195, y+30],...
    'numerator',['[' num2str(R) ']'], 'denominator','[1]', 'sampletime',str_Ts, 'orientation','left');
%all=get(get_param(block_R, 'handle'))
x=x-195;%R width
%add_line(name, 'Sum2/1', 'R(z)/1');
...
x=x-35;%spacing
add_block('simulink/Discrete/Discrete Filter', [nameofcont '/Hr(z)'], 'position',[x, y, x+195, y+30],...
    'numerator',['[' num2str(Hr) ']'], 'denominator','[1]', 'sampletime',str_Ts, 'orientation','left');
x=x-195;
add_line(nameofcont, 'R(z)/1', 'Hr(z)/1');
add_line(nameofcont, 'Hr(z)/1)', 'Sum1/2');
%add_line(name, 'R(z)/1)', 'Sum1/2');

add_block('simulink/Sinks/Out1', [nameofcont '/Outform'],'position',[1600, 100, 1620, 130])
add_block('simulink/Sinks/Out1', [nameofcont '/Outform2'],'position',[1600, 50, 1620, 80])
add_block('simulink/Sources/In1', [nameofcont '/into'],'position',[0, 100, 20, 130])
add_block('simulink/Sources/In1', [nameofcont '/into2'],'position',[1200, y-300, 1220, y-300+30])
add_line(nameofcont, '1//S(z)/1', 'Bp(z)//Ap(z)/1');
add_line(nameofcont, 'into/1', 'Bm(z)//Am(z)/1');
add_line(nameofcont, 'into2/1', 'Sum2/1');
add_line(nameofcont, 'Sum2/1', 'Outform/1');
add_line(nameofcont, 'Sum2/1', 'R(z)/1');
add_line(nameofcont, '1//S(z)/1', 'Outform2/1');


add_block('built-in/Subsystem', [name '/controller'],'position',[100, 85, 200, 200])
Simulink.BlockDiagram.copyContentsToSubsystem...
(nameofcont, [name +'/controller/'])
add_line(name, 'Disturbance Delay/1', 'controller/2');
add_line(name, 'Reference/1', 'controller/1','autorouting','smart');
%add_line(name, 'Sum2/1', 'controller/2','autorouting','smart');
add_line(name, 'controller/1', 'Mux/1');
if get(handles.checkbox1, 'value')%continuous plant
%add_line(name, 'controller/1', 'Zero-Order Hold/1','autorouting','smart');
else%discrete plant
%add_line(name, 'controller/1', 'Bp(z)//Ap(z)/1','autorouting','smart');
end


add_line(name, 'controller/2', 'Mux/2','autorouting','smart');


load_system(nameofcont)
cs = getActiveConfigSet(nameofcont);
cs.set_param('Solver', 'FixedStepauto');
switchTarget(cs,'ert.tlc',[])
set_param(nameofcont, 'ProdHWDeviceType', 'NXP->Cortex-M4')
rtwbuild(nameofcont)
...
function pushbutton5_Callback(hObject, eventdata, handles) %Generate Definition Files
% hObject    handle to pushbutton5 (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
[Ts Bp Ap Hr Hs dist P Bm Am]=acquire_data(handles);
%create file configration.h 
fileP=fopen('configration.h','w');
fprintf(fileP,'#ifndef _CONFIG_H \n#define _CONFIG_H_\n');
fclose(fileP);
fileP=fopen('configration.h','a');
fprintf(fileP,'#define delay= %f ;', str2num(get(handles.edit2,'string')));
fprintf(fileP,'\n#define Ts=%f ;', Ts);
fprintf(fileP,'\nextern const float B[%d], A[%d], P[%d],Am[%d],Bm[%d];',numel(Bp),numel(Ap),numel(Ap),numel(Am),numel(Bm));
fprintf(fileP,'\n#endif');
fclose(fileP);
%create configration.c
fileP=fopen('configration.c','w');
fprintf(fileP, '#include \"configration.h\"\n');
fprintf(fileP, 'const float P[%d]=',numel(P));
fprintf(fileP, '{');
for k=1:numel(P)-1
fprintf(fileP, [num2str(P(k)) ',']);
end
fprintf(fileP, [num2str(P(k+1)) '};\n']);
%B(z)
fileP=fopen('configration.c','a');
fprintf(fileP, 'const float B[%d]=',numel(Bp));
fprintf(fileP, '{');
for k=1:numel(Bp)-1
fprintf(fileP, [num2str(Bp(k)) ',']);
end
fprintf(fileP, [num2str(Bp(k+1)) '};\n']);
%A(z)
fileP=fopen('configration.c','a');
fprintf(fileP, 'const float A[%d]=',numel(Ap));
fprintf(fileP, '{');
for k=1:numel(Ap)-1
fprintf(fileP, [num2str(Ap(k)) ',']);
end
fprintf(fileP, [num2str(Ap(k+1)) '};\n']);
fclose(fileP);
%Am(z)
fileP=fopen('configration.c','a');
fprintf(fileP, 'const float Am[%d]=',numel(Am));
fprintf(fileP, '{');
for k=1:numel(Am)-1
fprintf(fileP, [num2str(Am(k)) ',']);
end
fprintf(fileP, [num2str(Am(k+1)) '};\n']);
fclose(fileP);
%Bm(z)
fileP=fopen('configration.c','a');
fprintf(fileP, 'const float Bm[%d]=',numel(Bm));
fprintf(fileP, '{');
for k=1:numel(Bm)-1
fprintf(fileP, [num2str(Bm(k)) ',']);
end
fprintf(fileP, [num2str(Bm(k+1)) '};\n']);
fclose(fileP);
...
%create file RST.h 
fileP=fopen('RST_parameters.h','w');
fprintf(fileP,'#ifndef _RST_H \n#define _RST_H_\n');
fclose(fileP);
fileP=fopen('RST_parameters.h','a');
fprintf(fileP,'extern const float R[%d], S[%d], T[%d];',numel(R),numel(S),numel(T));
fprintf(fileP,'\n#endif');
fclose(fileP);
%create RST.c
fileP=fopen('RST_parameters.c','w');
fprintf(fileP, '#include \"RST_parameters.h\"\n');
%R(z)
fprintf(fileP, 'const float R[%d]=',numel(R));
fprintf(fileP, '{');
for k=1:numel(R)-1
fprintf(fileP, [num2str(R(k)) ',']);
end
fprintf(fileP, [num2str(R(k+1)) '};\n']);
%S(z)
fileP=fopen('RST_parameters.c','a');
fprintf(fileP, 'const float S[%d]=',numel(S));
fprintf(fileP, '{');
for k=1:numel(S)-1
fprintf(fileP, [num2str(S(k)) ',']);
end
fprintf(fileP, [num2str(S(k+1)) '};\n']);
%T(z)
fileP=fopen('RST_parameters.c','a');
fprintf(fileP, 'const float T[%d]=',numel(T));
fprintf(fileP, '{');
for k=1:numel(T)-1
fprintf(fileP, [num2str(T(k)) ',']);
end
fprintf(fileP, [num2str(T(k+1)) '};\n']);
fclose(fileP);

% --- Executes on button press in pushbutton6.
function pushbutton6_Callback(hObject, eventdata, handles)%Generate Simulation C Files
...
%TS and samples
fileP=fopen('main.c','w');
fprintf(fileP,['#include <stdio.h>\n#include <stdlib.h>\nint main()\n{\nFILE * fp = fopen("mydata.csv", "w");\nif(fp == NULL){\nprintf("Couldnt open file");\nreturn;\n}\n']);
fprintf(fileP,['float Ts=' num2str(Ts) ';\n']);
fprintf(fileP,['int n_samples=' num2str(1000) ';\n']);
fprintf(fileP,['float Ref[' num2str(numel(T)) ']' ';\n']);
fprintf(fileP,['float Uk[' num2str(numel(S)) ']' ';\n']);
fprintf(fileP,['float Ek[' num2str(numel(Bp)) ']' ';\n']);
fprintf(fileP,['float yk[' num2str(max(numel(conv(S,A)),numel(R))) ']' ';\n']);
%initailize the arrays
fileP=fopen('main.c','a');
fprintf(fileP,['for(int i=1;i<' num2str(numel(T)) ';i++)Ref[i]=0;\n']);
fprintf(fileP,['for(int i=1;i<' num2str(numel(S)) ';i++)Uk[i]=0;\n']);
fprintf(fileP,['for(int i=1;i<' num2str(numel(Bp)) ';i++)Ek[i]=0;\n']);
fprintf(fileP,['for(int i=1;i<' num2str(max(numel(conv(S,A)),numel(R))) ';i++)yk[i]=0 ;\n']);
fclose(fileP);

%simulation loop
fileP=fopen('main.c','a');
fprintf(fileP,['for(int k=0;k<n_samples;++k)\n{\n//simulate\n']);
fprintf(fileP,['Ref[0]=1;\n']);
%to calc e(k)
fileP=fopen('main.c','a');
fprintf(fileP, 'Ek[0]=');
for k=1:numel(T)
if(T(k)>0)fprintf(fileP, ['+' num2str(T(k)) '*Ref[' num2str(k-1) ']']);
else
fprintf(fileP, [num2str(T(k)) '*Ref[' num2str(k-1) ']']); 
end
end

for k=1:numel(R)
if(R(k)>0)fprintf(fileP, ['+' num2str(R(k)) '*yk[' num2str(k-1) ']' ] );
else fprintf(fileP, [num2str(R(k)) '*yk[' num2str(k-1) ']' ] );
end
end
fprintf(fileP,';\n');
fclose(fileP);
...
fileP=fopen('main.c','a');
NUm=S/-S(1)
fprintf(fileP, 'Uk[0]=Ek[0]');
for k=1:numel(NUm)-1
if(NUm(k+1)>0)fprintf(fileP, ['+' num2str(NUm(k+1)) '*yk[' num2str(k) ']' ] );
else fprintf(fileP, [num2str(NUm(k+1)) '*yk[' num2str(k) ']' ] );
end
end
fprintf(fileP,';\n');
fclose(fileP);
...
fprintf(fileP, 'yk[0]=');
for k=1:numel(BE)
if(BE(k)>0)fprintf(fileP, ['+' num2str(BE(k)) '*Ek[' num2str(k-1) ']']);
else
fprintf(fileP, [num2str(BE(k)) '*Ek[' num2str(k-1) ']']); 
end
end
for k=1:numel(NUm)-1
if(NUm(k+1)>0)fprintf(fileP, ['+' num2str(NUm(k+1)) '*yk[' num2str(k) ']' ] );
...
fprintf(fileP,['printf(%cy[%c%c] = %c%c%c%c%c, k, yk[0]);\nfprintf(fp, %c%c%c,%c%c,%c%c,%c%c%c%c%c, k, k*Ts,-Uk[0],-yk[0]);\n'],'"','%','d','%','f','\n','"','"','%','d','%','f','%','f','%','f','\n','"');
fclose(fileP);

%shift old values
fileP=fopen('main.c','a');
fprintf(fileP,['for(int i=0;i<' num2str(numel(T)-1) ';i++)Ref[i+1]=Ref[i];\n']);
fprintf(fileP,['for(int i=0;i<' num2str(numel(S)-1) ';i++)Uk[i+1]=Uk[i];\n']);
fprintf(fileP,['for(int i=0;i<' num2str(numel(Bp)-1) ';i++)Ek[i+1]=Ek[i];\n']);
fprintf(fileP,['for(int i=0;i<' num2str(max(numel(conv(S,A)),numel(R))-1) ';i++)yk[i+1]=yk[i];\n}']);
fprintf(fileP,['fclose(fp);\nreturn 0;\n}']);
fclose(fileP);
